...
@test "yarn.lock v1: axios at safe version → OK" {
    cat > "$TMPDIR/yarn.lock" <<'EOF'
axios@^1.0.0:
  version "1.7.9"
...
EOF
    run bash "$SCRIPT" "$TMPDIR"
    [[ "$output" == *"[OK]   No malicious axios version"* ]]
}

@test "yarn.lock v1: no axios → OK" {
    cat > "$TMPDIR/yarn.lock" <<'EOF'
lodash@^4.0.0:
  version "4.17.21"
...
@test "yarn.lock v1: other package at 1.14.1, no axios → OK (no false positive)" {
    cat > "$TMPDIR/yarn.lock" <<'EOF'
some-package@^1.0.0:
  version "1.14.1"
...
@test "yarn.lock berry: other package at 1.14.1, no axios → OK (no false positive)" {
    cat > "$TMPDIR/yarn.lock" <<'EOF'
...
@test "package-lock.json: axios at safe version → OK" {
    cat > "$TMPDIR/package-lock.json" <<'EOF'
{
  "dependencies": {
    "axios": {
